...
from backend.ai.security_chat import (
    ask_security_copilot,
...
if "url_openapi_spec" not in st.session_state:
    st.session_state.url_openapi_spec = None

if "url_openapi_source" not in st.session_state:
    st.session_state.url_openapi_source = ""
...
def fetch_openapi_from_url(url):
    """
    Fetch an OpenAPI/Swagger document from a user-provided URL.

    The URL may point directly to an OpenAPI JSON document, or to an API
    base URL where a common documentation endpoint is available.
    """
    ...
    looks_like_spec = (
        ...
        or "swagger" in lower_url
        or "api-docs" in lower_url
    )

    if not looks_like_spec:
        candidates.extend(
            urljoin(url + "/", path.lstrip("/"))
    ...
    for candidate in candidates:
        ...
        try:
            ...
            try:
                ...
                errors.append(
                    f"{candidate} → response is not valid JSON"
                )
                continue

            if not isinstance(spec, dict):
                errors.append(
                    f"{candidate} → JSON root is not an object"
                )
                continue
            ...
            if is_openapi:
                return spec, str(response.url), errors

            errors.append(
                f"{candidate} → valid JSON, but no OpenAPI/Swagger paths found"
            )
...
st.markdown(
    '<div class="section-title">🌐 Scan API from URL</div><div class="section-bar"></div>',
    unsafe_allow_html=True
)
st.markdown(
    '<div class="section-caption">Paste an OpenAPI/Swagger URL, or an API base URL where SentinelAPI can discover a common OpenAPI documentation endpoint.</div>',
    unsafe_allow_html=True
)
...
with url_col1:
    openapi_url = st.text_input(
        "OpenAPI / Swagger URL",
        placeholder="https://example.com/openapi.json or https://example.com/api",
        key="openapi_url_input"
    )

with url_col2:
    discover_url = st.button(
        "🌐 Discover",
        use_container_width=True
    )

if discover_url:
    if not openapi_url.strip():
        st.error("❌ Please enter an OpenAPI/Swagger URL or API base URL.")
    else:
        with st.spinner("🔎 Discovering OpenAPI specification and endpoints..."):
            spec_from_url, resolved_url, discovery_errors = fetch_openapi_from_url(
                openapi_url.strip()
            )

        if spec_from_url is not None:
            st.session_state.url_openapi_spec = spec_from_url
            st.session_state.url_openapi_source = resolved_url

            detected_server = get_openapi_server(spec_from_url)
            if detected_server:
                st.session_state.api_base_url = detected_server
            else:
                st.session_state.api_base_url = urljoin(
                    resolved_url,
                    "."
                ).rstrip("/")

            url_endpoints = extract_endpoints(spec_from_url)
            st.session_state.uploaded_endpoints = url_endpoints
            st.session_state.uploaded_endpoint_count = len(url_endpoints)

            st.success(
                f"✅ API specification found · {len(url_endpoints)} endpoints discovered."
            )
            st.info(f"📡 Specification source: `{resolved_url}`")

            url_endpoint_rows = []
            for endpoint in url_endpoints:
                url_endpoint_rows.append(
                    {
            ...
            if url_endpoint_rows:
                st.markdown(
                    f'<div class="mini-label">🔗 URL Discovered Endpoints · {len(url_endpoint_rows)}</div>',
                    unsafe_allow_html=True
                )
                st.dataframe(
                    url_endpoint_rows,
            ...
            st.error("❌ Could not find a valid OpenAPI/Swagger document at the supplied URL.")
            if discovery_errors:
...
if scan_button:
    ...
    if not st.session_state.uploaded_endpoints:

        st.error(
            "❌ Please upload an OpenAPI JSON file or discover an API from URL first."
        )
...
if report is not None:
    ...
    st.markdown("---")
    st.markdown('<div class="section-title">🚨 API Security Overview</div><div class="section-bar"></div>', unsafe_allow_html=True)
    st.markdown(
        "**SentinelAPI scan summary:** total vulnerabilities, affected security areas, severity and affected endpoints."
    )

    total_findings = len(findings)

    # Group findings by vulnerability type. Keep the original finding data intact.
    vulnerability_groups = {}
    for finding in findings:
        vtype = finding.get("vulnerability_type", "Security Issue") or "Security Issue"
        if vtype not in vulnerability_groups:
            vulnerability_groups[vtype] = []
        vulnerability_groups[vtype].append(finding)
    ...
    with c1:
        st.metric("🚨 Total Vulnerabilities", total_findings)
    ...
    if total_findings == 0:
        st.success("✅ No security findings were generated by the completed scan.")
    else:
        st.warning(
            f"⚠️ SentinelAPI detected {total_findings} security finding(s). "
            "Review the affected endpoints and evidence below."
        )
    ...
    if vulnerability_groups:
        for vtype, group in vulnerability_groups.items():
            group_high = sum(1 for f in group if f.get("severity") == "HIGH")
            group_medium = sum(1 for f in group if f.get("severity") == "MEDIUM")
            group_low = sum(1 for f in group if f.get("severity") == "LOW")
            ...
            st.markdown(f"**🛡️ {vtype} — {len(group)} finding(s)**")

            risk_parts = []
            if group_high:
                risk_parts.append(f"HIGH: {group_high}")
            if group_medium:
                risk_parts.append(f"MEDIUM: {group_medium}")
            if group_low:
                risk_parts.append(f"LOW: {group_low}")

            st.write("Security risk: " + ", ".join(risk_parts))
            st.write("Affected endpoint(s): " + (", ".join(endpoints) if endpoints else "Not specified"))

    else:
        st.info("No vulnerability categories are available because no findings were generated.")

   

    st.markdown("### ⚠️ Security Risk Areas")

    risk_area_map = {
        "BOLA / IDOR": "Broken object authorization — another object's data may be accessible when an object ID is changed.",
        "Excessive Data Exposure": "Sensitive or unnecessary fields may be returned by an API response.",
        "Rate Limit Misconfiguration": "Repeated requests were accepted without the expected HTTP 429 rate-limit response.",
        "Authentication Misconfiguration": "An endpoint may be accessible without the authentication expected by the API specification.",
    }

    matched_risk_areas = []
    for vtype in vulnerability_groups:
        for key, explanation in risk_area_map.items():
            if key.lower() in vtype.lower() or vtype.lower() in key.lower():
                matched_risk_areas.append((key, explanation))
                break

    if matched_risk_areas:
        for name, explanation in matched_risk_areas:
            st.markdown(f"**🔸 {name}**")
            st.write(explanation)
    else:
        st.info("The scan did not generate a finding matching the currently displayed risk categories.")
    ...
    if not findings:
        ...
        severity_filter = st.selectbox(
            ...
            ["ALL", "HIGH", "MEDIUM", "LOW"],
            key="security_findings_severity_filter"
        )

        filtered_findings = [
            finding for finding in findings
            if severity_filter == "ALL" or finding.get("severity") == severity_filter
        ]
        ...
        for index, finding in enumerate(filtered_findings, start=1):
            ...
            if "BOLA" in vulnerability_type or "IDOR" in vulnerability_type:
                status_text = "⚠️ Potential issue — manual verification required"
                meaning = "Changing an object identifier returned another successful response. This is a BOLA/IDOR signal, but HTTP 200 alone does not prove unauthorized access."
            elif "Sensitive Data" in vulnerability_type or "Excessive Data" in vulnerability_type:
                status_text = "⚠️ Detected condition — review the returned data"
                meaning = "The response contains field names that may represent sensitive information and should be reviewed."
            elif "Authentication" in vulnerability_type:
                status_text = "⚠️ Review required — confirm whether public access is intended"
                meaning = "The endpoint may be accessible without authentication even though protection may be expected."
            elif "Rate Limit" in vulnerability_type:
                status_text = "⚠️ Potential issue — review endpoint requirements"
                meaning = "Repeated requests were accepted without the expected rate-limit response."
            else:
                status_text = "⚠️ Review required"
                meaning = description or "The scanner detected a condition that requires security review."

            with st.expander(f"{index}. {title} — {severity}"):
                st.markdown(f"**Status:** {status_text}")
                st.markdown(f"**Security risk:** {meaning}")
                st.markdown(f"**Type:** {vulnerability_type}")
                st.markdown(f"**Endpoint:** `{endpoint}`")
                st.markdown(f"**Description:** {description}")
                st.markdown(f"**Evidence:** {evidence}")
                st.markdown(f"**Recommendation:** {recommendation}")

                with st.expander("🧪 Reproduction / PoC", expanded=False):
                    st.code(finding_to_poc(finding), language="http")
...
st.markdown(
    ...
    ">
        ...
        <p style="color:#dbeafe; margin-top:8px;">
            Ask questions about your API security scan,
            vulnerabilities, affected endpoints and remediation.
        </p>
    </div>
    """,
    unsafe_allow_html=True
...
current_report = st.session_state.get("scan_report")

if current_report is None:

    st.info(
        "🔍 Run a security scan first. "
        "After the scan, Sentinel Security Copilot will "
        "analyze your findings."
    )

else:

    current_findings = current_report.get("findings", [])
    current_target = current_report.get("target", "")
    current_risk_score = current_report.get("risk_score", 0)

    current_summary = {
        "HIGH": current_report.get("high", 0),
        "MEDIUM": current_report.get("medium", 0),
        "LOW": current_report.get("low", 0)
    }

    security_context = build_security_context(
        target=current_target,
        endpoints=st.session_state.get(
            "uploaded_endpoints",
            []
        ),
    ...
    st.markdown("### ⚡ Quick Security Questions")
    ...
    with q1:
        if st.button(
            ...
            st.session_state.chat_question = (
                ...
                "Give me a clear summary with vulnerability type, "
                "severity and affected endpoint."
            )
    ...
    with q2:
        if st.button(
            ...
            st.session_state.chat_question = (
                "Explain all HIGH severity findings in this scan. "
                "For each one explain the problem, evidence, impact "
                "and how to fix it."
            )
    ...
    with q3:
        if st.button(
            ...
            st.session_state.chat_question = (
                "Create a practical remediation plan for all "
                "detected vulnerabilities in this scan."
            )
    ...
    with q4:
        if st.button(
            ...
            st.session_state.chat_question = (
                "Which endpoints are affected by the findings? "
                "Explain why each endpoint is risky."
            )
    ...
    with q5:
        if st.button(
            ...
            st.session_state.chat_question = (
                "Explain the BOLA/IDOR findings in this scan. "
                "Clearly distinguish possible findings from confirmed "
                "vulnerabilities and explain how to manually verify them."
            )
    ...
    with q6:
        if st.button(
            ...
            st.session_state.chat_question = (
                "Generate a prioritized technical remediation checklist "
                "for the findings in this scan."
            )
    ...
    user_question = st.chat_input(
        "Ask Sentinel Security Copilot about your API..."
    )
    ...
    if user_question:
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(
                "🤖 Sentinel Copilot is analyzing the scan..."
            ):

                try:

                    answer = ask_security_copilot(
                    ...
                    st.error(
                        f"❌ AI Security Copilot error: {error}"
                    )

   

    if st.button(
        "🗑️ Clear Security Chat",
        use_container_width=True
    ):
