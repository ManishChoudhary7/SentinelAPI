    ...
    if not findings:
        ...
        for index, finding in enumerate(
            ...
            if "BOLA" in vulnerability_type or "IDOR" in vulnerability_type:
                ...
                what_was_tested = (
                    "SentinelAPI found an API endpoint that uses an object ID "
                    "and tested whether changing that ID changes which object "
                    "can be accessed."
                )

                what_happened = (
                    "The scanner requested two different object IDs using the "
                    "same authentication context. Both requests returned a "
                    "successful HTTP response."
                )

                what_it_means = (
                    "This can be a BOLA/IDOR problem if the logged-in user is "
                    "not supposed to access the second object. However, a "
                    "successful HTTP 200 response by itself does NOT prove "
                    "that the access was unauthorized."
                )

                why_it_matters = (
                    "If authorization is missing, a user could potentially "
                    "change an object ID and access another user's data or "
                    "perform an action on an object they do not own."
                )

                next_action = (
                    "Verify the result with two different user accounts. "
                    "User A should try to access User B's object. If the API "
                    "returns the protected object instead of denying access, "
                    "the BOLA finding is confirmed."
                )
                ...
                what_was_tested = (
                    "SentinelAPI inspected successful API responses for field "
                    "names that may indicate sensitive information."
                )

                what_happened = (
                    "One or more response fields matched SentinelAPI's list of "
                    "potentially sensitive field names."
                )

                what_it_means = (
                    "The response may contain more sensitive information than "
                    "the client actually needs. The detected field still needs "
                    "to be checked to confirm whether it contains real sensitive data."
                )

                why_it_matters = (
                    "Unnecessary exposure of passwords, tokens, secrets or other "
                    "private information can increase the impact of an API compromise."
                )

                next_action = (
                    "Open the evidence, identify the returned fields, and confirm "
                    "whether each field is required and authorized for the current user."
                )
                ...
                what_was_tested = (
                    "SentinelAPI sent a request to the endpoint without an "
                    "authentication token."
                )

                what_happened = (
                    "The endpoint returned a successful response even though "
                    "no authentication token was supplied."
                )

                what_it_means = (
                    "The endpoint may be accessible without logging in. "
                    "This is only a problem if the endpoint is supposed to be protected."
                )

                why_it_matters = (
                    "An unintentionally public endpoint can expose information "
                    "or functionality that should only be available to authenticated users."
                )

                next_action = (
                    "Confirm whether the endpoint is intentionally public. If it "
                    "contains protected information or functionality, require authentication."
                )
                ...
                what_was_tested = (
                    "SentinelAPI sent repeated requests to the endpoint and "
                    "checked whether the API responded with HTTP 429 rate limiting."
                )

                what_happened = (
                    "The tested requests were accepted without receiving a "
                    "rate-limit response."
                )

                what_it_means = (
                    "The endpoint may not have sufficient request throttling. "
                    "The importance depends on what the endpoint does."
                )

                why_it_matters = (
                    "Without appropriate limits, automated clients may send a "
                    "large number of requests and consume resources or repeatedly "
                    "attempt sensitive operations."
                )

                next_action = (
                    "Check whether this endpoint needs rate limiting and define "
                    "an appropriate limit for the application's normal traffic."
                )
                ...
                what_was_tested = (
                    "SentinelAPI performed the security check associated with "
                    "this finding."
                )

                what_happened = description or (
                    "The scanner detected a condition that requires security review."
                )

                what_it_means = (
                    "The result indicates something that may affect the security "
                    "of this API, but the evidence should be reviewed before treating "
                    "it as a confirmed vulnerability."
                )

                why_it_matters = (
                    "The actual impact depends on the endpoint, data involved and "
                    "the authorization rules of the application."
                )

                next_action = (
                    "Review the evidence and reproduce the behavior in an authorized "
                    "test environment before making a final security decision."
                )
            ...
            with st.container():
                ...
                st.markdown(
                    ...
                    <div class="finding-card {severity_class}">
                        ...
                        <div>
                            <b>🧪 What did SentinelAPI test?</b><br>
                            {what_was_tested}
                        </div>
                        ...
                        <div>
                            <b>🔎 What did the scanner find?</b><br>
                            {what_happened}
                        </div>
                ...
                with st.expander("🧪 Reproduction / PoC"):

                    st.markdown(
                        """
                        **What is this section?**

                        This is the technical proof/reproduction information generated
                        by SentinelAPI. It is mainly for developers and security testers.

                        **Important:** A PoC shows what the scanner tested. It does not
                        automatically mean that the issue is a confirmed vulnerability.
                        Always verify the behavior against the application's authorization
                        rules.
                        """
                    )

                    st.code(
                        finding_to_poc(finding),
                        language="http"
                    )
